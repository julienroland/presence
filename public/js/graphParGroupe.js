...
part = {},
radius = 150,
canvas = document.getElementById("graphPresenceParGroupe"),
...
var getColors = function(  ){
	$.ajax({
		...
		success: function( data ) {
			oColorData = data.color.groupe;

			for(var i = 0;i <= oColorData.length-1;i++){

				myColors[oColorData[i].groupe] = oColorData[i].color;	
			}
		}
	})
...
var plotData = function() {
	...
	for (var i = 0; i < Object.keys(myData).length; i++) {
		...
		//GRAPH
		ctx.fillStyle = myColors[myData[i].groupe];
		ctx.beginPath();
		...
		ctx.fillText(myData[i].groupe+" ("+myData[i].percent+"%)", width+30, (height/height)+(i * 40)+20);
